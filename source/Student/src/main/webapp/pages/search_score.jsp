	...
	<center>
		<div class="window">
			<div class="searchbox tit">
				查找成绩： <select id="search_type">
					<c:choose>
						<c:when test="${sessionScope.log_operator.role.id == 3}">
							<option value="stu_all">查找全部</option>
							<option value="sub_name">科目名字</option>
							<option value="tec_name">科任教师</option>
						</c:when>
						<c:otherwise>
							<option value="stu_all">查找全部</option>
							<option value="stu_no">学生学号</option>
							<option value="stu_name">学生名字</option>
							<option value="sub_name">科目名字</option>
							<option value="cla_name">班级名字</option>
						</c:otherwise>
					</c:choose>
